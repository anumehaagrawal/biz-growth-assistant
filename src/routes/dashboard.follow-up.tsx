...
function FollowUpPage() {
  ...
  const loadContacts = useCallback(async () => {
    ...
    setContacts([...(data ?? []), ...getLocalContacts()]);
    setLoading(false);
  }, [user]);
  ...
  const addContact = async (event: React.FormEvent) => {
    ...
    if (error) {
      if (isMissingFollowUpTable(error.message)) {
        ...
        toast.success("Participant saved locally for this prototype");
        return;
      }
  ...
  const setStatus = async (id: string, follow_up_status: string) => {
    if (updateLocalContactStatus(id, follow_up_status)) {
      setContacts((current) =>
        current.map((contact) => (contact.id === id ? { ...contact, follow_up_status } : contact)),
      );
      return;
    }
  ...
  const completedCount = useMemo(
    () =>
      contacts.filter((contact) =>
        ["Visited again", "Joined", "Signup support offered"].includes(contact.follow_up_status),
      ).length,
    [contacts],
  );
      ...
      <section>
        <div className="grid gap-4 md:grid-cols-3">
          ...
          <Metric
            ...
            value={
              contacts.length ? `${Math.round((completedCount / contacts.length) * 100)}%` : "0%"
            }
          />
        </div>
        ...
        <div className="mt-5 space-y-4">
          {contacts.length === 0 && (
            <div className="rounded-3xl border border-dashed border-border bg-card p-10 text-center text-sm text-muted-foreground">
              Add a first visit here, or submit the family page visit form to start the tracker.
            </div>
          )}
...
function getLocalContacts(): FollowUpContact[] {
  if (typeof window === "undefined") return [];
  return JSON.parse(
    window.localStorage.getItem(localVisitRequestsKey) ?? "[]",
  ) as FollowUpContact[];
}

function saveLocalContacts(contacts: FollowUpContact[]) {
  if (typeof window === "undefined") return;
  window.localStorage.setItem(localVisitRequestsKey, JSON.stringify(contacts));
}

function updateLocalContactStatus(id: string, follow_up_status: string) {
  const contacts = getLocalContacts();
  if (!contacts.some((contact) => contact.id === id)) return false;
  saveLocalContacts(
    contacts.map((contact) => (contact.id === id ? { ...contact, follow_up_status } : contact)),
  );
  return true;
}

function removeLocalContact(id: string) {
  const contacts = getLocalContacts();
  if (!contacts.some((contact) => contact.id === id)) return false;
  saveLocalContacts(contacts.filter((contact) => contact.id !== id));
  return true;
}
      ...
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div>
          <div className="flex flex-wrap items-center gap-2">
            ...
            {!contact.user_id && <Badge>Visit request</Badge>}
          </div>
